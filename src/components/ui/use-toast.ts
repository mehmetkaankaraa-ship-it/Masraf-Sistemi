      ...
      if (toastId) {
        ...
        state.toasts.forEach((t) => {
          if (!toastTimeouts.has(t.id)) {
            toastTimeouts.set(t.id, setTimeout(() => { dispatch({ type: 'REMOVE_TOAST', toastId: t.id }); toastTimeouts.delete(t.id) }, TOAST_REMOVE_DELAY))
          }
        })
      }
      return { ...state, toasts: state.toasts.map((t) => ({ ...t, open: toastId ? t.id === toastId ? false : t.open : false })) }
    }
    case 'REMOVE_TOAST':
